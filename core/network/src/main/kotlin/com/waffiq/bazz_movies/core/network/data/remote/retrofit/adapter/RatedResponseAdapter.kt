 ...
 *
 * - [RatedResponse.Value]: Represents a rating value when the JSON contains a valid numeric `value` (float) field.
 ...
 *
 * This adapter avoids throwing exceptions on unexpected data by logging errors and returning a fallback value.
 * It ensures the app does not crash due to invalid API responses.
 */
class RatedResponseAdapter {
   ...
   *
   * @param rated The raw JSON value for the `rated` field. It could be a map, boolean, or unknown type.
  ...
  fun fromJson(rated: Any?): RatedResponse {
    return when (rated) {
      ...
      is Boolean -> {
        ...
        if (rated) RatedResponse.Unrated else RatedResponse.Unrated
      }
