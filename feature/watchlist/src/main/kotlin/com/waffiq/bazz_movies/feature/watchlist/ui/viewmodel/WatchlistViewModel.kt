        ...
        when (result) {
          FavoriteActionResult.Added -> {
            _snackBarAdded.value = Event(
              SnackBarUserLoginData(true, title, UpdateFavoriteParams(MOVIE_MEDIA_TYPE, id, true), null)
            )
          }
    ...
    launchAndHandleOutcome(
      ...
      onSuccess = { result ->
        when (result) {
          FavoriteActionResult.Added -> {
            _snackBarAdded.value = Event(
              SnackBarUserLoginData(true, title, UpdateFavoriteParams(TV_MEDIA_TYPE, id, true), null)
            )
          }
