...
class FlowJobHelperTest {
  ...
  @Before
  fun setup() {
    mockFragment = mockk<Fragment>(relaxed = true) {
      every { viewLifecycleOwner } returns lifecycleOwnerRule.lifecycleOwner
    }
  }

  @Test
  fun collectAndSubmitDataJob_whenLifecycleCreated_shouldSubmitPagingData() =
    runTest {
      collectAndSubmitDataJob(
